...
async function makeCallWithRetry(toNumber, fromNumber, systemMessage, campaignId, retries = 0) {
  try {
    // First try the local endpoint
    const response = await axios.post(`${process.env.PUBLIC_SERVER_URL}/initiate-call`, {
      to: toNumber,
      from: fromNumber,
    ...
    if (
      ...
      return makeCallWithRetry(toNumber, fromNumber, systemMessage, campaignId, retries + 1);
    }
...
async function processCampaignBatch(campaign) {
  ...
  try {
    ...
    console.log(`Processing batch of ${remainingNumbers.length} numbers for campaign ${campaign.id}`);

    // Get batch size based on available Telnyx numbers
    const batchSize = Math.min(campaign.telnyx_numbers.length, remainingNumbers.length);
    if (batchSize === 0) {
      console.log(`Campaign ${campaign.id} completed - no more numbers to call`);
    ...
    // Make calls for current batch
    const calls = currentBatch.map((toNumber, index) => {
      const fromNumber = campaign.telnyx_numbers[index];
      console.log(`Initiating call from ${fromNumber} to ${toNumber}`);
      return makeCallWithRetry(toNumber, fromNumber, campaign.system_message, campaign.id);
    });
  ...
  try {
    ...
    switch (data.event_type) {
      ...
      case "call.completed":
        ...
        if (call.campaign_id) {
          await Campaign.increment('completed_calls', { where: { id: call.campaign_id } });
        }
        break;
      ...
      case "call.no-answer":
        ...
        // Update campaign stats
        if (call.campaign_id) {
          await Campaign.increment('failed_calls', { where: { id: call.campaign_id } });
        }
        break;
    }
    ...
    if (call.campaign_id && ["completed", "failed", "no-answer"].includes(call.status)) {
      ...
      if (campaign && campaign.status === "in_progress") {
        ...
        if (activeCalls === 0 && campaign.numbers_to_call.length > 0) {
          console.log(`All calls in batch completed, starting next batch for campaign ${campaign.id}`);
